<p align="center">
<kbd>
   <img src="./assets/images/banner.png" alt="PLEXILE ARCADE" style="border-radius: 60%; width: 415px;height: auto;">
</kbd>
</p>

<p align="center">Play instantly, no downloads, no sign up.</p>

<p align="center">
  <img src="https://img.shields.io/github/stars/PLEXILENetwork/PLEXILEARCADE?style=for-the-badge&color=C93131" alt="Stars">
  <img src="https://img.shields.io/github/issues/PLEXILENetwork/PLEXILEARCADE?style=for-the-badge&color=C93131" alt="Issues">
</p>

---

## Credits

List of assets used during the development of PLEXILE ARCADE.

> [!IMPORTANT]  
> PLEXILE ARCADE does NOT own any of these games. Full credits go to their original creators and respective owners.

```bash
gn-math.dev: 17% of game assets
```
```bash
selenite.cc: 15% of game assets
```
```bash
3kh0.net: 8% of game assets
```
```bash
classroom-6x.io: 60% of game assets (cdn)
```

Made with ❤️ by j (PLEXILE Network)